...
- **OpenNext Build Chain (CRITICAL):** The `build` script in `package.json` MUST NOT include `opennextjs-cloudflare build`. Per [official OpenNext docs](https://opennext.js.org/cloudflare), `opennextjs-cloudflare build` internally runs `npm run build`, so placing it inside `build` causes infinite recursion. The correct approach is: (1) keep `build` as `npm run cf-typegen && next build`, (2) add `[build] command = "npx opennextjs-cloudflare build"` in `wrangler.toml` so Wrangler auto-runs it before deploy — this is the Poka-Yoke that ensures `.open-next/worker.js` always exists for CI.
