...
- **OpenNext Build Chain (CRITICAL):** The `build` script in `package.json` MUST NOT include `opennextjs-cloudflare build`. The `opennextjs-cloudflare build` command internally calls `npm run build`, so adding it to the `build` script causes an infinite recursion loop. Instead, the CI/CD pipeline should run `npm run build` and then `npx opennextjs-cloudflare build` sequentially, or use `npx opennextjs-cloudflare deploy`.
