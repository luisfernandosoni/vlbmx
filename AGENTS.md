...
- **OpenNext Build Chain (CRITICAL):** The `build` script in `package.json` MUST NOT include `opennextjs-cloudflare build`. Per [official OpenNext docs](https://opennext.js.org/cloudflare), `opennextjs-cloudflare build` internally runs `npm run build`, so placing it inside `build` causes infinite recursion. Correct setup: (1) `package.json build` = `npm run cf-typegen && next build` only, (2) Cloudflare Workers Builds CI **Build command** (dashboard setting) = `npx opennextjs-cloudflare build`, (3) local deploy uses `npm run deploy` which explicitly chains `opennextjs-cloudflare build && opennextjs-cloudflare deploy`. Do NOT use `[build]` in `wrangler.toml` — it fires on ALL wrangler commands including `wrangler types`, causing unintended OpenNext builds during type generation.
- **Playwright Stealth & OOM (CRITICAL):** Always disable `navigator.webdriver` and use `playwright-extra` + `puppeteer-extra-plugin-stealth` for premium platform scraping (VBTV, Roku). On Hetzner CAX21 (8GB), strictly limit concurrent Playwright instances to max 3-4 (they consume 400MB-1.5GB each), and MUST mount `/dev/shm` with >1GB in Docker configuration to prevent Chromium from crashing due to shared memory exhaustion.
- **Cloudflare D1 Bulk Inserts (CRITICAL):** NEVER use `Promise.all` or loops for multi-row inserts in Cloudflare D1. ALWAYS use Drizzle ORM's `db.batch()` or `.values(array)` for bulk operations. D1 is SQLite over HTTP; batching 1000 rows reduces latency by up to 10x.
- **Durable Objects WebSockets (CRITICAL):** NEVER use the traditional `.addEventListener` pattern for handling WebSocket events inside a Durable Object. You MUST use the Hibernatable API: provide `async webSocketMessage()` and `async webSocketClose()` class methods, and accept connections via `this.ctx.acceptWebSocket(server)`.
- **Better Auth Server-to-Server (CRITICAL):** When utilizing Better Auth for pure server-to-server validation (e.g., Cloudflare ↔ Hetzner), strictly use the `apiKey()` plugin. Define an `apiKeyGetter` or specific `apiKeyHeaders` to extract and validate the key on the receiver without requiring a browser session.
